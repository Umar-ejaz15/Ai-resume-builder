...
export default function ResumeCardItem({ item, index, refreshList }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await resumeApi.deleteResume(item.documentId || item.id);
  ...
  const resumeId = item.documentId || item.id;
      ...
      <div className="absolute bottom-0 left-0 w-full flex justify-between items-center px-4 py-3 bg-purple-800/90 backdrop-blur-sm">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-48 bg-white shadow-lg rounded-xl p-2 space-y-2">
            ...
            <DropdownMenuItem asChild>
              <Link href={`/dashboard/resume/${resumeId}/view`}>
